...
window.SidebarController = {
  ...
  renameRecentChat(chatId, triggerButton) {
    const chat = this.getRecentChats().find(item => item.id === chatId);
    if (!chat) return;

    const row = triggerButton?.closest('.sidebar-chat-row, .sidebar-search-result-row');
    if (!row) return;

    const input = document.createElement('input');
    input.className = 'sidebar-inline-rename-input';
    input.type = 'text';
    input.maxLength = 64;
    ...
    const finish = (shouldSave) => {
      ...
      if (shouldSave && nextTitle) {
        this.commitRecentChatRename(chatId, nextTitle);
        return;
      }
      this.renderRecents();
      this.renderConversationSearchResults?.();
  ...
  commitRecentChatRename(chatId, nextTitle) {
    const cleanTitle = nextTitle.trim().slice(0, 64);
    if (!cleanTitle) return;
    this.updateRecentChat(chatId, { title: cleanTitle });
    if (window.HomeController?.currentChatId === chatId) {
      window.HomeController.currentChatTitle = cleanTitle;
      window.HomeController.updateChatHeader?.();
    }
  ...
  renderRecents() {
    ...
    flatWrap.querySelectorAll('[data-rename-chat-id]').forEach(button => {
      button.addEventListener('click', (event) => {
        event.stopPropagation();
        this.renameRecentChat(button.dataset.renameChatId, button);
      });
    });
  ...
  renderConversationSearchResults(query = this.searchQuery || '') {
    ...
    results.querySelectorAll('[data-search-rename-chat-id]').forEach(button => {
      button.addEventListener('click', (event) => {
        event.stopPropagation();
        this.renameRecentChat(button.dataset.searchRenameChatId, button);
      });
    });
      ...
      modal.innerHTML = `
        <div class="settings-dialog" role="dialog" aria-modal="true" aria-labelledby="settings-title">
          ...
          <div class="settings-grid">
            ...
            <label>Default model<select id="settings-model"><option value="Juno 2.5">Default</option><option>Research Analyst</option><option>Procurement Assistant</option><option>Customer Support Agent</option><option>Data Interpreter</option><option>Document Writer</option><option>Workflow Planner</option></select></label>
