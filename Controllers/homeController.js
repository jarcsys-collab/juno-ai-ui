...
window.HomeController = {
  ...
  async submitMessage(message) {
    ...
    this.ensureCurrentConversation(message, files);
  ...
  ensureCurrentConversation(message = '', files = []) {
    ...
    if (!this.currentChatTitle) {
      const firstInput = message.trim() || files[0]?.shortName || files[0]?.name || '';
      this.currentChatTitle = firstInput ? this.getChatTitle(firstInput) : 'Default';
    }
  ...
  saveCurrentChatToRecents() {
    ...
    const firstUserMessage = history.querySelector('.chat-row[data-message-role="user"] .message-content')?.textContent.trim() || '';
    this.ensureCurrentConversation(firstUserMessage);
  ...
  wireChatToolbar() {
    ...
    titleInput?.addEventListener('keydown', event => {
      if (event.key === 'Enter') {
        event.preventDefault();
        this.finishActiveChatRename(true);
      }
      if (event.key === 'Escape') {
        event.preventDefault();
        this.finishActiveChatRename(false);
      }
    });
    titleInput?.addEventListener('blur', () => this.finishActiveChatRename(true));
  ...
  renameActiveChat() {
    ...
    const currentTitle = this.currentChatTitle || this.getChatTitle(history.textContent) || 'Default';
    const titleGroup = document.getElementById('chat-title-group');
    const titleInput = document.getElementById('chat-title-input');
    if (!titleGroup || !titleInput || titleGroup.classList.contains('is-renaming')) return;

    titleInput.value = currentTitle;
    titleGroup.classList.add('is-renaming');
  ...
  finishActiveChatRename(shouldSave) {
    const titleGroup = document.getElementById('chat-title-group');
    const titleInput = document.getElementById('chat-title-input');
    if (!titleGroup?.classList.contains('is-renaming') || !titleInput) return;

    titleGroup.classList.remove('is-renaming');
    if (!shouldSave || !titleInput.value.trim()) {
      this.updateChatHeader();
      return;
    }

    this.currentChatTitle = titleInput.value.trim().slice(0, 64);
    this.updateChatHeader();
    this.saveCurrentChatToRecents();
  ...
  updateChatHeader() {
    ...
    if (meta) {
      meta.textContent = [
        ...
        this.activeAgent === 'Juno 2.5' ? 'Default' : (this.activeAgent || 'Default')
      ].filter(Boolean).join(' · ');
    }
  },
  ...
  wireFilesPanel() {
    ...
    if (filesClose && filesPanel) {
      filesClose.addEventListener('click', () => {
        filesPanel.style.display = 'none';
        if (filesToggle) filesToggle.style.display = 'flex';
      });
    }
  ...
  updateModelPills(nextAgent) {
    document.querySelectorAll('.model-pill').forEach(pill => {
      pill.textContent = nextAgent === 'Juno 2.5' ? 'Default' : nextAgent;
